...
void Solver<Dtype>::TestDetection(const int test_net_id) {
  ...
  for (int i = 0; i < all_true_pos.size(); ++i) {
    ...
    map<int, float> APs;
    map<int, float> recalls;//(Added by xun)
    map<int, float> precisions;//(Added by xun)
    float mAP = 0.;
    ...
    for (map<int, int>::const_iterator it = num_pos.begin();
      ...
      // Add output file (Added by xun)
      std::ofstream ofresult_p("/home/dawn/precision.txt",ios::app);
	  for(std::vector<float>::iterator iter = prec.begin(); iter != prec.end(); ++iter)
	  {
	  ofresult_p<<*iter<<std::endl;
	  }
	  ofresult_p.close();
	  
      std::ofstream ofresult_r("/home/dawn/recall.txt",ios::app);
	  for(std::vector<float>::iterator iter = rec.begin(); iter != rec.end(); ++iter)
	  {
	  ofresult_r<<*iter<<std::endl;
	  }
	  ofresult_r.close();
	  
      mAP += APs[label];
      
      // Compute precision and recall under threshold 'thr'(Added by xun)
      float thr = param_.rec_prec_thr();
      int tp_sum = 0;
      int fp_sum = 0;
      for(int i = 0; i < label_true_pos.size(); ++i) {// tp num
          if(label_true_pos[i].first > thr) {
              tp_sum += label_true_pos[i].second;
          }
      }
      recalls[label] = static_cast<float>(tp_sum) / label_num_pos;
      for(int i = 0; i < label_false_pos.size(); ++i) {// fp num
